...
const CalendarApp = (Props) => {
  ...
  function CheckAtteance(){
    const CurrentLocation = "1"
    if (Props.Location == CurrentLocation) {
      alert("cheking Backend");
    }
    else{
      alert("Your Not in lecture");
    }
  }
            ...
            <div className="DatesInMonth  grid grid-cols-7 border-b-2 border-[#3838386d] gap-2 p-4 divide-y divide-[#3838386d]">
              ...
              {[...Array(M_EndDay).keys()].map((day) => {
                ...
                return (
                  <span
                    key={day + 1}
                    className={`flex items-center justify-center h-15 w-25 rounded-lg hover:bg-[#E98074] ${isToday ? "bg-[#E85A4F] text-white font-bold rounded-lg" : ""}`}
                  >
                    {day + 1}
                  </span>
                );
